...
router.post('/analyze-cv', upload.single('resume'), async (req, res) => {
  ...
  try {
    ...
    const filteredJobs = jobsArray.filter(job => {
      if (filtersObj.type && filtersObj.type !== 'All' && job.type !== filtersObj.type) {
        return false;
      }
      return true;
    });
    ...
    const jobsToAnalyze = filteredJobs.length > MAX_JOBS_TO_ANALYZE
      ? filteredJobs.slice(0, MAX_JOBS_TO_ANALYZE)
      : filteredJobs;
    
    console.log(`Analyzing ${jobsToAnalyze.length} jobs (out of ${filteredJobs.length} filtered, ${jobsArray.length} total)`);

    // Call Gemini API for analysis
    const analysis = await analyzeCVWithJobs(resumeText, jobsToAnalyze, filtersObj);
    ...
    const errorMessage = error.message || 'An error occurred during CV analysis.';
    res.status(500).json({ 
      error: errorMessage,
